...
class SEModule(Module):
    def __init__(self, channels, reduction):
        ...
        # Use flat naming for compatibility with pretrained weights
        self.se_fc1 = Conv2d(
            channels, channels // reduction, kernel_size=1, padding=0, bias=False
        )
        self.se_bn1 = BatchNorm2d(channels // reduction)
        self.relu = ReLU(inplace=True)

        self.se_fc2 = Conv2d(
            channels // reduction, channels, kernel_size=1, padding=0, bias=False
        )
        self.se_bn2 = BatchNorm2d(channels)
...
class Depth_Wise_SE(Module):
    def __init__(
        ...
        # Replace SEModule with direct definition (matches weight keys)
        self.se_fc1 = Conv2d(c3_out, c3_out // se_reduct, kernel_size=1, bias=False)
        self.se_bn1 = BatchNorm2d(c3_out // se_reduct)
        self.relu = ReLU(inplace=True)
        self.se_fc2 = Conv2d(c3_out // se_reduct, c3_out, kernel_size=1, bias=False)
        self.se_bn2 = BatchNorm2d(c3_out)
        self.sigmoid = Sigmoid()
...
def MiniFASNetV2(
    embedding_size=128, conv6_kernel=(7, 7), drop_p=0.0, num_classes=3, img_channel=3
):
    return MiniFASNet(
        keep_dict["1.8M_"],
        embedding_size,
